from drawingDimensioning.core import __dir__
from drawingDimensioning.command import *
from drawingDimensioning.centerLines import _centerLineSVG
import numpy
...
def unfold(faces_org, face_names):
    faces = [ FaceWrapper(f,n) for f,n in zip( faces_org, face_names ) ]
...
class FaceWrapper:
    def __init__(self, freeCAD_face_object, subElementName):
        self.fC_face = freeCAD_face_object
        self.name = subElementName
...
def edge_value_t( edge, t ):
    assert hasattr( edge, 'Curve')
    v =  edge.FirstParameter + t*( edge.LastParameter - edge.FirstParameter )
    #print(v, edge.LastParameter - edge.FirstParameter, edge.Length )
    return tuple( edge.valueAt(v) )
...
class Projection:
    ...
    def _draw(self, faceWrapper):
        ...
        for i, edge in enumerate(face.Edges):
            if str(edge.Curve).startswith('<Line'):
                try:
                    P = [
                        transform( edge_value_t( edge, 0) ),
                        transform( edge_value_t( edge, 1) )
                    ]
                except Exception as msg:
                    raise Exception( '%s.Edge[%i]: "%s"' % ( faceWrapper.name, i, msg) )
    ...
    def unfold( self, faceWrapper ):
        ...
        faceWrapper.transform = pTransform2(
            ...
            edge_value_t(cEdge, 0),
            edgeToConnectTo.faceW.transform( edge_value_t(cEdge, 0) ).posProjection,
            edge_value_t(cEdge, 1),
            edgeToConnectTo.faceW.transform( edge_value_t(cEdge, 1) ).posProjection,
        )
        self._draw( faceWrapper ) 
...
class pPoint: #p prefix denoties projection
    def __init__(self, x, y, pos3D):
        assert abs(x) < 12345, 'x out of range %3.2e, issue with transform?' % x
        assert abs(y) < 12345, 'y out of range %3.2e, issue with transform?' % y
        self.x = x
        self.y = y
...
class FoldingLine:
    ...
    def svg(self, strokeWidth, lineColor, len_dash, len_gap):
        assert norm( self.p1[0:2] - self.p2[0:2] ) / ( len_dash + len_dash + len_gap ) < 1000, "FoldingLine: too many parts self.p1 %s, self.p2 %s" % (self.p1, self.p2)
...
class UnfoldCommand:
    def Activated(self):
        ...
        if len(selection) == 1 and all( isinstance(s, Part.Face) for s in selection[0].SubObjects )  :
            ...
            d.projection = unfold( selection[0].SubObjects, selection[0].SubElementNames )
