...
def halfLinearDimensionSVG( x1, y1, x2, y2, x3, y3, x4=None, y4=None, scale=1.0, textFormat='%3.3f',  gap_datum_points = 2, dimension_line_overshoot=1, arrowL1=3, arrowL2=1, arrowW=2, svgTag='g', svgParms='', strokeWidth=0.5, lineColor='blue', textRenderer=defaultTextRenderer):
    lines = []
    ...
    B = p2 + numpy.dot(p3-p2,d)*d
    #lines.append( dimensionSVG_trimLine( p1, A, gap_datum_points,-dimension_line_overshoot)) #linePerpendic1
    lines.append( dimensionSVG_trimLine( p2, B, gap_datum_points,-dimension_line_overshoot)) #linePerpendic2
    #lines.append( A.tolist() +  B.tolist() ) #line between the two
    ...
    if distAB > 0:
        #s = 1 if distAB > 2.5*(arrowL1 + arrowL2) else -1
        s = 1 # for half line dim, arrow is always inside
        #arrowXML = arrowHeadSVG( A, s*directionVector(A,B), arrowL1, arrowL2, arrowW, lineColor ) + \
        #    arrowHeadSVG( B, s*directionVector(B,A), arrowL1, arrowL2, arrowW, lineColor )
