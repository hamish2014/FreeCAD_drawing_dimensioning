...
class DimensioningWorkbench (Workbench): 
    ...
    def Initialize(self):
        ...
        for module in ['linearDimension', 'deleteDimension', 'circularDimension', 'textAdd', 'textEdit', 'escapeDimensioning', 'angularDimension', 'partsList']:
            if not DEBUG:
                importlib.import_module( module )
            else:
                crudeDebugger.printingDebugging( os.path.join(__dir__, module + '.py') )
                importlib.import_module(  module + '_crudeDebugging')
        commandslist = ['linearDimension', 'circularDimension', 'angularDimension', 'textAddDimensioning','textEditDimensioning', 'deleteDimension', 'escapeDimensioning', 'addPartsList']
